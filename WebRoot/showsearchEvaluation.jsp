			...
			<div id="page">
	          <div class="wrapper">				
					<table id="report" class="stylized full" >
						...
						<tbody>
						    <s:set name="servevas" value="#request.servevas"></s:set> 
						     <s:iterator value="#servevas" id="serveva">
							<tr>
								<td class="ta-right"><s:property value="#serveva.ordernumber"/></td>
								<s:if test="#serveva.status==0">
								      <td class="ta-right">未评价
								           <a href="<s:url action="getSEva">
                                           <s:param name="ordernumber" value="ordernumber" /></s:url>">请评价</a>
								      </td>
								</s:if>
								<s:if test="#serveva.status==1">
								      <td class="ta-right">已评价
								           <a href="<s:url action="getSEva1">
                                           <s:param name="ordernumber" value="ordernumber" /></s:url>">修改评价</a>
								      </td>
								</s:if>
								<td class="ta-right"><s:property value="#serveva.score"/></td>
								<td class="ta-right"><s:property value="#serveva.information"/></td>
							</tr>
							</s:iterator>
						</tbody>
					</table>
